...
def nonOptimalSolver():
    NonOptimalSolver.solve(initState.board, n)
...
def optimalSolver():
    ...
    while(completed(boardArray[0].board)!=1):
        ...
        if (len(boardArray) > 15000 and boardArray[0].hamming()-boardArray[0].moves<5):
            print("Oops! The optimal solver is taking too long to compute.\nSwitching to Non Optimal Solver...")
            #print(boardArray[0].hamming()-boardArray[0].moves)
