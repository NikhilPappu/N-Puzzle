...
def Down(i,j,val,row,board,n):
    moveBlank(i+1,j,val,row,board,n)
    swap(board,i+1,j,i,j)
def placeRowElement(val,row,board,n,col):
    newVal = val % n
    if (newVal == 0): newVal = n
...
def placeRowN(val,row,board,n):
    ...
    while (i > row + 1):
        ...
        Up(i, j, val, row, board, n)
        i -= 1
    #if (find(val, board, n)[0] != row or find(val, board, n)[1] != newVal - 1): return
    moveBlank(i+1,j,val,row,board,n)
...
def solve(board,n):
    ...
    (zi,zj) = find(0,board,n)
    while(zi!=n-1 or zj!=n-1 or find(n*n-1,board,n)[0]!=n-1 or find(n*n-1,board,n)[1]!=n-2):
        if(zi==n-1 and zj==n-2):
            swap(board,zi,zj,zi-1,zj)
            zi-=1
        elif(zi==n-2 and zj==n-2):
            swap(board,zi,zj,zi,zj+1)
            zj+=1
        elif(zi==n-2 and zj==n-1):
            swap(board,zi,zj,zi+1,zj)
            zi+=1
        elif(zi==n-1 and zj==n-1):
            swap(board,zi,zj,zi,zj-1)
            zj-=1
